...
relevantni_produkty = {produkt_id: score for produkt_id, score in zip(products["id"], cosine_similarities) if score > 0}

serazene_produkty = sorted(relevantni_produkty.items(), key=lambda item: item[1], reverse=True)

# Vypsani vysledku
for produkt_id, score in serazene_produkty:
    print(f"Produkt ID: {produkt_id}, Skóre: {score}")
    print(products.iloc[produkt_id - 1]["název"], "|", products.iloc[produkt_id - 1]["popis"], "|",  products.iloc[produkt_id - 1]["cena"], "|",  products.iloc[produkt_id - 1]["recenze"], "\n")